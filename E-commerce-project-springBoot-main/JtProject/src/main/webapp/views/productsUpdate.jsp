			...
			<div class="row">
				<div class="col-sm-5">
					...
					try {
						...
						ResultSet rs = stmt.executeQuery("SELECT id, name, categoryid, price, quantity, description, image FROM products WHERE id = " + pid);
					...
					<div class="form-group">
						...
						<input type="text" class="form-control border border-warning" required name="name" value="<%= rs.getString(2) %>">
					</div>
					...
					<div class="form-group">
						...
						<select class="form-control border border-warning" name="categoryid" required >
							...
							{
								%>
								<option value="<%= rs2.getInt(1) %>" <% if (rs.getInt(3) == rs2.getInt(1)) { %> selected <% } %>><%= rs2.getString(2) %></option>
								<%	
							}
							%>
						</select>
					...
					<div class="form-group">
						...
						<input type="number" class="form-control border border-warning" name="price" step="any" value="<%= rs.getDouble(4) %>" required>
					</div>
					<div class="form-group">
						<label for="quantity">Available Quantity</label> 
						<input type="number" class="form-control border border-warning" name="quantity" min="1" value="<%= rs.getInt(5) %>" required>
					</div>
				...
				<div class="form-group">
						...
						<textarea class="form-control border border-warning" rows="4" name="description" ><%= rs.getString(6) %></textarea>
					</div>
					<div class="form-group">
					  <label for="productImage">Image URL</label>
					  <textarea class="form-control border border-warning" rows="2" name="productImage"><%= rs.getString(7) %></textarea>
					</div>
