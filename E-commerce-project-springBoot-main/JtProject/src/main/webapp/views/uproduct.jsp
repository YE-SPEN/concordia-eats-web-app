...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
	...
	<%
		...
		String currentPage = "redirect:/user/products";
	%>
			...
			<tr style="cellpadding: 10;">
				<td>					
						<form method="post" action="/search">
	        				...
	        				<input class="btn btn-info" style="background-color: #fff; color: #222222; text-align: left;" type="text" name="search" placeholder="Search by Product Name">
	        				<button class="btn btn-info" style="background-color: #DDDDDD; color: #222222" type="submit"><b>Search</b></button>
	    				</form>
	    		</td>
				<td><h5>&nbsp;&nbsp;&nbsp;Filter by...&nbsp;&nbsp;&nbsp;</h5></td>
				<td scope="col">					
						<form method="post" action="/filter">
							<input type="hidden" name="filterType" value="discounts">
							<input type="hidden" name="id" value="1">						
	        				<button class="btn btn-info" style="background-color:<% if ( filterType != null && filterType.equals("discounts")) { %> #339933 <% } else { %> #fff; color: #222222 <% } %>" type="submit">On Discount</button>
	    				</form>
	    		</td>
				<td scope="col">
						<form method="post" action="/filter">
							<input type="hidden" name="filterType" value="favorites">
							<input type="hidden" name="id" value="1">						
	        				<button class="btn btn-info" style="background-color:<% if ( filterType != null && filterType.equals("favorites")) { %> #339933 <% } else { %> #fff; color: #222222 <% } %>" type="submit">My Favorites</button>
	    				</form>
				</td>
				<td scope="col">
					<form method="post" action="/filter">
						<input type="hidden" name="filterType" value="category">
							<select class="btn btn-info" style="background-color: #fff; color: #222222; text-align: left;" name="id" required>
									<option value="">Filter by Category</option>
								<%
								String url = "jdbc:mysql://localhost:3306/springproject";
								Class.forName("com.mysql.cj.jdbc.Driver");
								Connection con = DriverManager.getConnection(url, "root", "");
								Statement getCategories = con.createStatement();
								ResultSet allCategories = getCategories.executeQuery("select * from categories");
								while(allCategories.next())
								{
									%>
									<option value="<%= allCategories.getString(1) %>"><%= allCategories.getString(2) %></option>
								<% } 
								%>
							</select>						
	        				<button class="btn btn-info" style="background-color:<% if ( filterType != null && filterType.equals("category")) { %> #339933 <% } else { %> #DDDDDD; color: #222222 <% } %>" type="submit"><b>Select</b></button>
	   				</form>
	   			</td>
	   			<td scope="col">
	   				&nbsp;&nbsp;&nbsp;<a href="/user/products"><b>Clear Filters</b></a>
	   			</td>		
			</tr>
			<tbody>
			</tbody>
				...
				<tr>
					...
					try {
					    ...
					    if (filterType != null) {
					        if (filterType.equals("category")) {
					            ...
					            currentPage += "?filter=category&id=" + categoryId;
					            rs = stmt.executeQuery("SELECT * FROM products WHERE categoryid =" + categoryId + " AND quantity > 0");
					        } 
					        else if (filterType.equals("favorites")) {
					            currentPage += "?filter=favorites";
					            rs = stmt.executeQuery("SELECT * FROM products p, favorites f WHERE p.id = f.product_id AND f.user_id =" + uid + " AND quantity > 0");
					        } 
					        
					        else if (filterType.equals("discounts")) {
					            currentPage += "?filter=discounts";
					            rs = stmt.executeQuery("SELECT * FROM products WHERE onDiscount = 1 AND quantity > 0");
					        }
					        else if (filterType.equals("search")) {
								rs = (ResultSet)session.getAttribute("search");
					        }
					    } 
					...
					<td>
						...
						<%								
							...
							if(recResult.next()) { %>
								<img src="https://467210.fs1.hubspotusercontent-na1.net/hubfs/467210/recommended-badge-1.png" width=90px">
						<%
						} %>
					</td>
